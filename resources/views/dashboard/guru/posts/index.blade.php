    ...
    <section class="content">
        <div class="container-fluid">
                ...
                <div class="btn-toolbar mb-2 mb-md-0">
                    <a href="{{url('guru/posts/create')}}" class="btn btn-primary me-2 mb-3 mr-3">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-sm">
                    ...
                    <tbody>
                        ...
                        @if($post->user_id ==  Auth::guard('guru')->user()->id )
                        <tr class="text-center">
                            <td><img src="{{ asset('images/'.$post->photo) }}" class="img-fluid rounded" width="180px" alt="{{$post->photo}}"></td>
                            <td><p>{{ $post->title }}</p></td>
                            <td>{!! substr($post->body,0,96) !!}..</td>
                            <td><a href="{{ url('guru/posts/'.$post->slug)}}" class="btn btn-primary"><i class="far fa-eye"></i></a></td>
                            <td><a href="{{ url('guru/posts/'.$post->slug.'/edit')}}" class="btn btn-warning"><i class="far fa-edit"></i></a></td>
                            <td>
                                <form action="{{ url('guru/posts/'.$post->id)}}" method="post" class="d-inline">
                                    @method('delete')
                                    @csrf
